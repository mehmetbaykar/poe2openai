...
use crate::types::*;
use crate::utils::{
    convert_poe_error_to_openai, count_completion_tokens, count_message_tokens, format_bytes_length, format_duration, get_cached_config, process_message_images,
};
use chrono::Utc;
...
async fn handle_replace_response(
    ...
    let final_content = {
        ...
        for (ref_id, file_data) in file_refs_map.iter() {
            ...
            debug!("🖼️ 處理圖片引用 | 標記: {} | 替換為: {}", img_marker, replacement);
        }
        
        // 檢查是否有圖片引用被替換
        if processed_content != replace_content {
            debug!("✅ 成功替換圖片引用 | 最終內容長度: {}", format_bytes_length(processed_content.len()));
        } else if !file_refs_map.is_empty() {
            warn!("⚠️ 有圖片引用但未找到對應標記 | 圖片數: {}", file_refs_map.len());
        }
